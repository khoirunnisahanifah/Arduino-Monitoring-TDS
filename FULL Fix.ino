...
//define sensor TDS
#define PinSensorTDS A0 //menetapkan Pin TDS A0
#define VREF          3.3 // analog reference voltage(Volt) of the ADC
#define SCOUNT        5  // sum of sample point

int analogBuffer[SCOUNT];    // store the analog value in the array, read from ADC
int analogBufferTemp[SCOUNT];
int analogBufferIndex = 0,IndexCopy = 0;
float rataratavoltase = 0,tdsValue = 0,temperature = 25;
unsigned long int analogSampleTimepoint,printTimepoint;


//inisialisasi connect mqtt
const char* server = "broker.emqx.io";
const char* port_mqtt = 1883;

//inisialisasi konek wifi
char ssid[] = "Iphone Ifa";           // your network SSID (name)
char pass[] = "gayabangetlu";           // your network password
...
void tdsprosess(){
  ...
  if(millis()-analogSampleTimepoint>40)     //read the analog value from the ADC
 {
   ...
   analogBuffer[analogBufferIndex]=analogRead(PinSensorTDS);    //read the analog value and store into the buffer
   analogBufferIndex++;
 ...
 if(millis()-printTimepoint>1500)
 {
    ...
    for(IndexCopy=0;IndexCopy<SCOUNT;IndexCopy++) analogBufferTemp[IndexCopy]=analogBuffer[IndexCopy];
    rataratavoltase=getMedianNum(analogBufferTemp,SCOUNT)*(float)VREF/1024.0; // read the analog value more stable by the median filtering algorithm, and convert to voltage value
    float kompensasikoefisien=1.0+0.02*(temperature-25.0);    //temperature compensation formula: fFinalResult(25^C) = fFinalResult(current)/(1.0+0.02*(fTP-25.0));
    float kompensasiVoltase=rataratavoltase/kompensasikoefisien;  //temperature compensation
    tdsValue=(133.42*kompensasiVoltase*kompensasiVoltase*kompensasiVoltase-255.86*kompensasiVoltase*kompensasiVoltase+857.39*kompensasiVoltase)*0.7; //convert voltage value to tds value
